...
## 16. Fázis: Android EAS Build konfiguráció
A 16. fázis fejlesztései során előkészítésre került a React Native (Expo) mobilalkalmazás natív Android buildje az Expo Application Services (EAS) segítségével.
- **EAS Konfiguráció:** Telepítésre került az `eas-cli` csomag a `mobile` projekt függőségei közé, és elkészült az `eas.json` konfigurációs fájl, amely támogatja az Android platformot és az APK fájlok preview generálását.
- **Android Specifikus Beállítások:** Az `app.json` fájl kiegészült a `"package": "com.pohanka.smartshopper"` egyedi csomagazonosítóval az Android build sikeres lefuttatásához.
- **EAS Build parancsok:**
  - Helyi EAS CLI telepítés és inicializálás:
    ```bash
    npm install --save-dev eas-cli
    ```
  - EAS build elindítása Android platformra (preview APK):
    ```bash
    npx eas build --platform android --profile preview
    ```
  - EAS build elindítása Android platformra (produkciós bundle):
    ```bash
